...
FluidSystem::FluidSystem ()
{
	mNumPoints = 0;
	mMaxPoints = 0;
	mPackBuf = 0x0;   // pointer to the array of particles ?  not used ?
	mPackGrid = 0x0;
	mbRecord = false;
...
void FluidSystem::Start ( int num )     // #### creates the particles ####
{
	...
	// Setup stuff
	AllocatePackBuf ();   // allocates mPackBuf (particle buffer ?) ...  not used. Particles are in m_Fluid.bufC(pos/vel/veval/force/press/density/clr)
	SetupKernels ();
	SetupSpacing ();
...
void FluidSystem::SetGenome (FGenome newGenome )
{
    ...
    for(int i=0; i< NUM_GENES; i++) for(int j=0; j< NUM_GENES; j++){  m_FGenome.sensitivity[i][j] = newGenome.sensitivity[i][j];}
...
{
    std::cout << "starting FluidSystem::Exit (),   MAX_BUF = " << MAX_BUF << std::flush; 
	// Free fluid buffers
...
int FluidSystem::AddParticleMorphogenesis ()
{
    ...
    *(m_Fluid.bufI(FCLR) + n) = 0;
    ...
    for(int j=0;j<(BONDS_PER_PARTICLE +1);j++) { ElastIdx[j] = 0; }
...
int FluidSystem::AddParticleMorphogenesis (Vector3DF* Pos, Vector3DF* Vel, uint Age, uint Clr, uint *_ElastIdx, uint NerveIdx, uint* _Conc, uint* _EpiGen )
{
    ...
    *(m_Fluid.bufI(FCLR) + n) = Clr;
    ...
    for(int j=0;j<(BONDS_PER_PARTICLE +1);j++) { ElastIdx[j] = _ElastIdx[j] ; }
    
    //fprintf(fp, "\t%u, \t", *NerveIdx); 
    *(m_Fluid.bufI(FNERVEIDX) + n) = NerveIdx;
    
    uint* Conc = (m_Fluid.bufI(FCONC) + n);
    for(int j=0;j<(NUM_TF);j++) { Conc[j] = _Conc[j]; }
     
    uint* EpiGen = (m_Fluid.bufI(FEPIGEN) + n);    
    for(int j=0;j<(NUM_GENES);j++) { EpiGen[j]= _EpiGen[0]; }
...
void FluidSystem::SavePointsCSV ( const char * relativePath, int frame )
{
    ...
    for(int i=0;i<numpnt;i++){
        ...
        for(int j=0;j<(NUM_TF);j++) { fprintf(fp, "%u, ",  Conc[j] ); }
        fprintf(fp, "\t"); 
        
        for(int j=0;j<(NUM_GENES);j++) { fprintf(fp, "%u, ",  EpiGen[j] ); }
        fprintf(fp, " \n"); 
    }
	fclose ( fp );
	fflush ( fp );
...
void FluidSystem::ReadPointsCSV ( const char * relativePath , int gpu_mode, int cpu_mode)    // NB allocates buffers as well.
{
    ...
    for (i=1; i<number_of_lines; i++ ){
        ...
        for(int j=0;j<(BONDS_PER_PARTICLE +1);j++) { ret += std::fscanf(points_file, "%u, ",  &ElastIdx[j] ); }
        ret += std::fscanf(points_file, "\t%u, \t", &NerveIdx); 
        
        for(int j=0;j<(NUM_TF);j++) { ret += std::fscanf(points_file, "%u, ",  &Conc[j] ); }
        ret += std::fscanf(points_file, "\t"); 
        
        for(int j=0;j<(NUM_GENES);j++) { ret += std::fscanf(points_file, "%u, ",  &EpiGen[j] ); }
        ret += std::fscanf(points_file, " \n"); 
     /////////////////////////////   
