...
//	using namespace nvdb;
    ...
    // 32bit unsigned int for Uid - upto 4,294,967,296 particles
    // 32bit float for modulus (could be more complex e.g. fatigue, nonlinear) (lookup type)
    // 128 = 4*32 extra bits/particle (upto 3 'bonds) 
    ...
    #define BONDS_PER_PARTICLE  3   // +1 for self ID, and t
    ...
    // 128bits
    #define NUM_TF              16      //  minimum 8 ranscription factors + 8 diffusable morphogens
...
// original buffers continued    
	...
	struct FBufs {                                 // holds an array of pointers, and functions to access them
		#ifdef CUDA_KERNEL
